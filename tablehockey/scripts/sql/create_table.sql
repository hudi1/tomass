...
CREATE TABLE IF NOT EXISTS `stage` (
  ...
  `TYPE` enum('RR','PO') NOT NULL,
  `TOURNAMENT_ID` int(11) NOT NULL,
  `STATUS` enum('NS','BC','F') NOT NULL DEFAULT 'NS',
  `COPY_RESULT_STAGE` int(11) DEFAULT NULL,
  PRIMARY KEY (`ID`),
  KEY `TOURNAMENT_ID` (`TOURNAMENT_ID`),
  KEY `COPY_RESULT_STAGE` (`COPY_RESULT_STAGE`),
  KEY `NAME` (`NAME`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 AUTO_INCREMENT=10 ;
...
ALTER TABLE `stage`
  ...
  ADD CONSTRAINT `STAGE_ibfk_2` FOREIGN KEY (`COPY_RESULT_STAGE`) REFERENCES `stage` (`ID`) ON DELETE CASCADE ON UPDATE CASCADE;  
